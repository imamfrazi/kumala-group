    ...
    <section class="content">
      ...
      <div class="row mt-2 mb-2">
        ...
        <div class="container-fluid">
          <div class="row">
            <div class="col-sm-6">
              <h1 class="m-0 text-dark">Mining</h1>
            </div>
            <!-- /.col -->
          </div>
          <!-- /.row -->
        ...
        <div class="col-lg-12 col-xs-12">
         <div class="card">
          ...
          <div class="card-body table-responsive p-0">
                ...
                <tr v-for="mining in minings.data" :key="mining.id">
                  <td>{{mining.id}}</td>
                  <td>{{mining.nama}} / {{mining.name}}</td>
                  <td>
                    <img :src="'img/mining/' + mining.gambar" width="150px" alt>
                  </td>
                  <td>{{mining.deskripsi}}</td>
                  <td>{{mining.desc}}</td>
                  <td>{{mining.quantity}}</td>
                  <td>{{mining.price}}</td>
                  <td>
                    <a @click="deletemining(mining.id)" class="btn btn-social-icon btn-pinterest">
                       <i class="fa fa-trash red"></i>
                    </a>
                    <a @click="editmining(mining)" class="btn btn-social-icon btn-dropbox">                       
            ...
            <div class="card-footer clearfix">
              <pagination align="right" :data="minings" @pagination-change-page="getResults"></pagination>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    load() {
      axios.get("api/mining").then(({ data }) => (this.minings = data));
    },
    deletemining(id) {
      swal({
        title: "Are you sure?",
    ...
    getResults(page = 1) {
      axios.get("api/mining?page=" + page).then(response => {
        this.minings = response.data;
      });
    },
    editmining(mining) {
      this.form.reset();
      this.form.fill(mining);
      this.editmode = true;
    },
    update() {
      this.$Progress.start();
      this.form
        .put("api/mining/" + this.form.id)
        .then(() => {
          $("#addNew").modal("hide");
    ...
    create() {
      ...
      this.form
        ...
        .then(() => {
          ...
          toast({
            type: "success",
            title: "Berhasil membuat mining baru"
          });
          this.$Progress.finish();
        })
        .catch(e => {
  ...
  created() {
    Fire.$on('searching',() => {
        ...
        axios.get('api/findmining?q=' + query)
        .then((data) => {
            this.minings = data.data
        })
        .catch(() => {
